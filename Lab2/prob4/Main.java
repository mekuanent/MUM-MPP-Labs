...
public class Main {	
	...
	private List<String> getCourseNames(Student s) {
		List<TranscriptEntry> all = s.getGrades(); //s.grades;
	...
	private List<String> getGrades(Section s) {
		...
		for(TranscriptEntry t : s.getGradeSheet()/*s.gradeSheet*/) {
			grades.add(t.grade);
		}
		return grades;
	}
	private List<String> getStudentsWith(String grade) {
		...
		for(Student s : students) {
			boolean found = false;
			for(TranscriptEntry te : s.getGrades()/*s.grades*/) {
				if(!found) {
	...
	private void readDataFromDb() {
		...
		bob = factory.createStudent("Bob", "1");//.name = "Bob";
		tim = factory.createStudent("Tim", "1");//.name = "Tim";
		allen = factory.createStudent("Allen", "1");//.name = "Allen";
		bio1 = factory.createSection("Biology", 1); //.sectionNumber = 1;
		bio2 = factory.createSection("Biology", 2); //.sectionNumber = 2;
		math = factory.createSection("Math", 3); //.sectionNumber = 3;
